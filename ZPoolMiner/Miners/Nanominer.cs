...
namespace ZPoolMiner.Miners
{
    public class Nanominer : Miner
    {
        ...
        private string GetStartCommand(string wallet, string ID, string password)
        {
            ...
            if (MiningSetup.CurrentAlgorithmType.Equals(AlgorithmType.VerusHash))
            {
                ...
                cfgFile =
                   ...
                   + String.Format("rigName = {0}", ID.Trim()) + "\n"
        ...
        protected override string BenchmarkCreateCommandLine(Algorithm algorithm, int time)
        {
            ...
            if (algorithm.PrimaryAlgorithmPoolID == AlgorithmType.VerusHash)
            {
                var cfgFile =
                   string.Format("webPort = {0}", ApiPort) + "\n"
                   + string.Format("mport = 0\n")
            ...
            if (algorithm.PrimaryAlgorithmPoolID == AlgorithmType.KawPow)
            {
                var cfgFile =
                   String.Format("webPort = {0}", ApiPort) + "\n"
                   + String.Format("mport = 0\n")
