        ...
        public void SwichMostProfitableGroupUpMethod(object sender, SmaUpdateEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (var device in _miningDevices)
                {
                    ...
                    {
                        if (c.timesincelast + 600 > device.Device.coinMiningTime && device.diffPercent <= cointreshold)
                        {
                        ...
                        if (c.timesincelast + 600 < device.Device.coinMiningTime &&
                            coinFail.Contains(device.Device.Coin))
                        {
                            Helpers.ConsolePrint(Tag, "Block found for " + device.Device.Coin + " " +
                                c.timesincelast.ToString() + " - " + device.Device.coinMiningTime.ToString());
                ...
                foreach (var device in _miningDevices)
                {
                    ...
                    {
                        if (algo.PrimaryAlgorithmPoolID == AlgorithmType.KawPowLite)
                        {
                        ...
                        {
                            if ((AlgorithmType)device.Device.AlgorithmID == algo.PrimaryAlgorithmPoolID ||
                                (AlgorithmType)device.Device.SecondAlgorithmID == algo.DualZPoolID)
                            {
                                algoZero = true;
                    ...
                    */
                    ...
                    var _c = Stats.Stats.CoinList.Find(a => (a.algo.ToLower() == device.CurrentProfitableAlgorithmType.ToString().ToLower() &&
                    a.symbol.ToLower() == device.DeviceCurrentMiningCoin.ToString().ToLower()));
                    if (_c is object && _c != null)
                    {
                        if (_c.tempBlock)
                        {
                            ...
                            Helpers.ConsolePrint("MiningSession", "Need switch " + ((AlgorithmType)device.Device.AlgorithmID).ToString() + " " +
                                device.DeviceCurrentMiningCoin + " -> " + device.DeviceMostProfitableCoin +
                                " because coin is blocked or inactive");
                            if (coinFail.Exists(a => a.ToLower() == device.DeviceCurrentMiningCoin.ToLower()))
                            {
                    ...
                    */
                    ...
                    if ((double.IsInfinity(device.diff) ||
                        double.IsInfinity(device.diffPercent)) && !needSwitch)//api bug
                    {
                        needSwitch = true;
                        forceSwitch = true;
                        device.needSwitch = false;
                    ...
                    if (percDiff > SwitchProfitabilityThreshold && coinChanged)
                    {
                        ...
                        if (_ticks[0] + 1 >= AlgorithmSwitchingManager._ticksForStable || forceSwitch)
                        {
                            ...
                            else
                            {
                                ...
                                */
                                if (percDiff <= 10 || double.IsInfinity(percDiff) || double.IsNaN(percDiff) || forceSwitch || coinChanged)
                                {
                                    actualProfit = 0d;
                                    localProfit = 0d;
                                    foreach (var __algoProperty in Stats.Stats.algosProperty)
                        ...
                        */
                            ...
                            !device.DeviceCurrentMiningCoin.Equals(device.DeviceMostProfitableCoin))
                        {
                                ...
                                forceSwitch)
                            {
                                ...
                                {
                                    if (percDiff <= 10 || double.IsInfinity(percDiff) || forceSwitch ||
                                        coinChanged)
                                    {
                                        Helpers.ConsolePrint(Tag,
...
[2025-10-09 11:58:30] [INFO] [SWITCHING] Number of switches: 15 Uptime: 0 days 18:20:14
        ...
        private AlgorithmType GetMinerPairAlgorithmType(List<MiningPair> miningPairs)
        {
            ...
            {
                return miningPairs[0].Algorithm.DualZPoolID;
            }
