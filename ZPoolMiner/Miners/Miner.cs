    ...
    public abstract class Miner
    {
        ...
        private void SetApiPort()
        {
            if (IsInit)
            {
                ...
                var algoType = MiningSetup.MiningPairs[0].Algorithm.PrimaryAlgorithmPoolID;
        ...
        public abstract void Start(string wallet, string ID, string password);
        ...
        protected void KillProcessAndChildren(int pid)
        {
            ...
            try
            {
                ...
                if (IsProcessRunning(pid) && _p is object && _p != null)
                {
                    //Helpers.ConsolePrint("KillProcessAndChildren", "CloseMainWindow " + _p.ProcessName + " " + _p.Id.ToString());
                    _p.CloseMainWindow();
                    _p.Kill();
                }
                Thread.Sleep(200);
                if (IsProcessRunning(pid) && _p is object && _p != null)
                {
                    //Helpers.ConsolePrint("KillProcessAndChildren", "Close");
                    _p.Close();
                }
                Thread.Sleep(200);
                if (IsProcessRunning(pid) && _p is object && _p != null)
                {
                    //Helpers.ConsolePrint("KillProcessAndChildren", "Kill");
                    _p.Kill();
                }
                Thread.Sleep(200);
                if (IsProcessRunning(pid) && _p is object && _p != null)
                {
                    //Helpers.ConsolePrint("KillProcessAndChildren", "Dispose");
                    _p.Dispose();
                }
                Thread.Sleep(200);
            }
        ...
        protected void StopMiner(MinerStopType willswitch)
        {
            var algo = (int)MiningSetup.CurrentAlgorithmType;
            string strPlatform = "";
        ...
        public virtual void BenchmarkStart(int time, IBenchmarkComunicator benchmarkComunicator)
        {
            ...
            _benchmarkLogPath =
                ...
                $"{MiningSetup.MiningPairs[0].Algorithm.SecondaryAlgorithmPoolID}";
        ...
        protected virtual Process BenchmarkStartProcess(string commandLine)
        {
            ...
            if (benchmarkHandle.StartInfo.FileName.ToLower().Contains("srbminer") &&
                (commandLine.ToLower().Contains("firopow")))
            {
                benchmarkHandle.StartInfo.FileName = benchmarkHandle.StartInfo.FileName.Replace("SRBMiner-MULTI.exe", "SRBMiner-MULTI269.exe");
            }
            if (benchmarkHandle.StartInfo.FileName.ToLower().Contains("srbminer") &&
            ...
            */
            ...
            benchmarkHandle.StartInfo.WindowStyle = ProcessWindowStyle.Hidden | ProcessWindowStyle.Minimized;
            ...
            //Form_Benchmark.ActiveForm.Focus();
            if (benchmarkHandle.StartInfo.FileName.Contains("cmd.exe"))
            {
                ...
                if (benchmarkHandle.StartInfo.Arguments.Contains("miniz22c"))
                {
                    ...
                    gp = benchmarkHandle.SetChildPid("miniz22c");//store pID
                    _currentPidData = new MinerPidData
                    {
                        MinerBinPath = "miniz22c.exe",
                        Pid = gp
                    };
    ...
    $"Trying to kill benchmark process {BenchmarkProcessPath} algorithm {BenchmarkAlgorithm.AlgorithmName}");
            ...
            if (BenchmarkAlgorithm is DualAlgorithm dualAlg)
            {
                Helpers.ConsolePrint(MinerTag(),
                    "Final Speed: " + Helpers.FormatDualSpeedOutput(BenchmarkAlgorithm.BenchmarkSpeed,
                        BenchmarkAlgorithm.BenchmarkSecondarySpeed, 0, dualAlg.PrimaryAlgorithmPoolID, dualAlg.DualZPoolID) + 
            ...
            else
            {
                Helpers.ConsolePrint(MinerTag(),
                    "Final Speed: " + Helpers.FormatDualSpeedOutput(BenchmarkAlgorithm.BenchmarkSpeed, 0, 0,
                        BenchmarkAlgorithm.PrimaryAlgorithmPoolID, BenchmarkAlgorithm.DualZPoolID) + 
                        " Power: " + BenchmarkAlgorithm.PowerUsageBenchmark.ToString("F0"));
            }
        ...
        protected virtual void BenchmarkThreadRoutine(object commandLine)
        {
            ...
            try
            {
                ...
                if (BenchmarkAlgorithm.SecondaryAlgorithmPoolID != AlgorithmType.NONE && 
                ...
                if (BenchmarkAlgorithm.DualZPoolID == AlgorithmType.VertHash)
                {
            ...
            (BenchmarkHandle.GetChildPid() > 0 && IsActiveProcess(BenchmarkHandle.GetChildPid())))
            {
                try
                {
                        ...
                        || BenchmarkException != null)
                    {
                        if (BenchmarkSignalQuit)
                        {
                            Helpers.ConsolePrint(MinerTag(), "Benchmark quit");
                        }
                        if (BenchmarkSignalFinnished)
                        {
                            Helpers.ConsolePrint(MinerTag(), "Benchmark finished");
                        }
                        if (BenchmarkSignalFinnished)
                        {
                            Helpers.ConsolePrint(MinerTag(), "Benchmark hanged");
                        }
                        ...
                        {
                            Helpers.ConsolePrint(MinerTag(), "BenchmarkException");
                        }
                ...
                if (!IsActiveProcess(BenchmarkHandle.Id))
                {
                    //Helpers.ConsolePrint(MinerTag(), "Miner process not exist");
                }
            }
        ...
        private void StopBenchmark(Process BenchmarkHandle)
        {
            try
            {
                if (BenchmarkHandle.StartInfo.FileName.ToLower().Contains("cmd") &&
            ...
            {
                Helpers.ConsolePrint("StopBenchmark*", ex.ToString());
            }
        }
        ...
        private void DetectLiteMode()
        {
            ...
            foreach (var pair in MiningSetup.MiningPairs)
            {
                ...
                if (algo.PrimaryAlgorithmPoolID == AlgorithmType.KawPowLite && algo.Enabled)
                {
        ...
        protected virtual MinerProcess _Start()
        {
            ...
            if (MiningSetup.MinerPath.ToLower().Contains("srbminer") &&
                (LastCommandLine.ToLower().Contains("firopow")))
            {
                Path = MiningSetup.MinerPath.Replace("SRBMiner-MULTI.exe", "SRBMiner-MULTI269.exe");
            }
            if (MiningSetup.MinerPath.ToLower().Contains("srbminer") &&
            ...
            */
            ...
            else if(MiningSetup.MinerPath.ToLower().Contains("miniz") && 
                (LastCommandLine.ToLower().Contains("evrprogpow")))
            {
                //
            }
            ...
            if (LastCommandLine.Contains("config_zp"))
            {
                try
                {
                    ...
                    coin = f.Substring(f.IndexOf(",zap=") + 5).Split(' ')[0].Trim();
                } catch (Exception ex)
                {
                }
            }
            ...
            {
                coin = LastCommandLine.Substring(LastCommandLine.IndexOf(",zap=") + 5).Split(' ')[0].Trim();
            }
            try
            {
                string strPlatform = "";
                foreach (var pair in MiningSetup.MiningPairs)
                {
                    int a = (int)pair.Algorithm.PrimaryAlgorithmPoolID;
                    int b = (int)pair.Algorithm.SecondaryAlgorithmPoolID;
        ...
        protected Process RunCMDBeforeOrAfterMining(bool isBefore)
        {
            ...
            try
            {
                ...
                foreach (var pair in MiningSetup.MiningPairs)
                {
                    if ((int)pair.Algorithm.DualZPoolID <= -10)
                    {
                        strDual = "DUAL";
                    }
                    if (pair.Device.DeviceType == DeviceType.NVIDIA)
