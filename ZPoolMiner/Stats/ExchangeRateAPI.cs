...
namespace ZPoolMiner.Stats
{
    internal static class ExchangeRateApi
    {
        ...
        private static string GetAPI(string url)
        {
            ...
            try
            {
                ...
                {
                    WebProxy proxy = new WebProxy(Stats.CurrentProxy.HostName, Stats.CurrentProxy.HTTPSPort);
                    WR.Proxy = proxy;
                }
                //WR.UserAgent = "GET / HTTP/1.1\r\n\r\n";
        ...
        public static ConcurrentDictionary<string, double> GetRatesgateio()
        {
            ...
            if (ConfigManager.GeneralConfig.EnableProxy)
            {
                Helpers.ConsolePrint("ExchangeRateApi", "Trying get cryptocurrency exchange rates from Gate.io " +
                    "via proxy " + Stats.CurrentProxy.HostName + ":" + Stats.CurrentProxy.HTTPSPort.ToString());
            } else
            {
                Helpers.ConsolePrint("ExchangeRateApi", "Trying get cryptocurrency exchange rates from Gate.io");
            }
    ...
    Links.cbrURL);
        ...
        public static double ConvertBTCToPayoutCurrency(double btc)
        {
            ...
            /*
            if (!Form_Main._demoMode)
            {
                MessageBox.Show("Unknown Currency Tag: " + ConfigManager.GeneralConfig.PayoutCurrency + ". Contact the developer!",
