        ...
        "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            private static class Cpu
            {
                public static void QueryCpus()
                {
                    ...
                    // get all CPUs
                    Available.CpusCount = CpuUtils.PhysicalProcessorCount();
                    int LogicalCPUCount = Environment.ProcessorCount;
                    Helpers.ConsolePrint(Tag, "Number Of CPU Sockets: " + Available.CpusCount.ToString());
                    Helpers.ConsolePrint(Tag, "Number Of Threads: " + LogicalCPUCount.ToString());

                    int coreCount = CpuUtils.GetCoresCount();

                    Helpers.ConsolePrint(Tag, "Number Of Cores: " + coreCount.ToString());

                    if (LogicalCPUCount > coreCount)
                    {
                        Available.IsHyperThreadingEnabled = true;
                    } else
                    {
                        Available.IsHyperThreadingEnabled = false;
                    }
                    ...
                    if (CpuUtils.IsCpuMiningCapable())
                    {
                        if (Available.CpusCount == 1)
                        {
                            Available.Devices.Add(
                                new CpuComputeDevice(0, "CPU0", CpuUtils.GetCpuName().Trim(), threadsPerCpu, 0,
                                    ++CpuCount, false)
                            );
                        }
                        else if (Available.CpusCount > 1)
                        {
                            ...
                            {
                                Available.Devices.Add(
                                    new CpuComputeDevice(i, "CPU" + i, CpuUtils.GetCpuName().Trim(), threadsPerCpu,
                                        ComputeDevice.CreateAffinityMask(i, threadsPerCpuMask), ++CpuCount)
                                );
                            }
                        }
                    }
