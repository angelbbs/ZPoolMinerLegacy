...
namespace ZPoolMiner.Devices.Algorithms
{
    /// <summary>
...
*/
            ...
            */
                ...
                (device.Codename.ToLower().Contains("ellesmere")))
            {
                foreach (var algo in algoSettings[MinerBaseType.SRBMiner])
                {
                    if (algo.DualZPoolID == AlgorithmType.KarlsenHashV2HooHash)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
            ...
            */
            /*
            if (device.DeviceType == DeviceType.AMD && algoSettings.ContainsKey(MinerBaseType.GMiner) &&
                (device.Codename.ToLower().Contains("gfx1201")))//RX 9070 XT
            {
                foreach (var algo in algoSettings[MinerBaseType.GMiner])
                {
                    if (algo.DualZPoolID == AlgorithmType.KawPow)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
            ...
            */
                ...
                (device.Codename.ToLower().Contains("gfx1201")))//RX 9070 XT
            {
                foreach (var algo in algoSettings[MinerBaseType.SRBMiner])
                {
                    if (algo.DualZPoolID == AlgorithmType.Curve)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
                    if (algo.DualZPoolID == AlgorithmType.Yescrypt)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
                    if (algo.DualZPoolID == AlgorithmType.YescryptR16)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
                    if (algo.DualZPoolID == AlgorithmType.YescryptR32)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
                    if (algo.DualZPoolID == AlgorithmType.YescryptR8)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
                ...
                device.Codename.ToLower().Contains("vii")))
            {
                ...
                {
                    if (algo.PrimaryAlgorithmPoolID == AlgorithmType.NeoScrypt)
                    {
                        algo.Enabled = false;
                        algo.Hidden = true;
                    }
            ...
            */
            ...
            if (algoSettings.ContainsKey(MinerBaseType.CryptoDredge)) //not supported
            {
                ...
                {
                    if (device.DeviceType == DeviceType.NVIDIA && 
                        (algo.DualZPoolID == AlgorithmType.NeoScrypt || algo.DualZPoolID == AlgorithmType.SHA256csm) &&
            ...
            if (algoSettings.ContainsKey(MinerBaseType.CryptoDredge)) //not supported
            {
                ...
                {
                    if (device.DeviceType == DeviceType.NVIDIA &&
                        (algo.DualZPoolID == AlgorithmType.NeoScrypt || 
                        algo.DualZPoolID == AlgorithmType.SHA256csm ||
                        algo.DualZPoolID == AlgorithmType.Hmq1725 ||
                        algo.DualZPoolID == AlgorithmType.Allium) &&
            ...
            IList<MinerBaseType> toRemoveBase = null)
        {
            ...
            if (toRemoveBase == null)
            {
                ...
                foreach (var kvp in minerAlgos)
                {
                    var algoList = kvp.Value.FindAll(a => toRemove.IndexOf(a.DualZPoolID) == -1);
                    if (algoList.Count > 0)
                    {
