...
namespace ZPoolMiner
{
    class Firewall
    {
        ...
        public static void AddToFirewall()
        {
            ...
            SetFirewallRule($"advfirewall firewall add rule name=mlff program={Directory.GetCurrentDirectory() + "\\ZPoolMinerLegacy.exe"} protocol=tcp dir=in enable=yes action=allow");
            SetFirewallRule($"advfirewall firewall add rule name=mlff program={Directory.GetCurrentDirectory() + "\\ZPoolMinerLegacy.exe"} protocol=tcp dir=out enable=yes action=allow");
        }
    }
}
