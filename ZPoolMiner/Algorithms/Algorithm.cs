...
namespace ZPoolMiner.Algorithms
{
    public class Algorithm
    {
        ...
        /// </summary>
        public readonly AlgorithmType PrimaryAlgorithmPoolID;
        /// <summary>
        ...
        public virtual AlgorithmType SecondaryAlgorithmPoolID => AlgorithmType.NONE;
        public virtual AlgorithmType DualZPoolID => PrimaryAlgorithmPoolID;

        #endregion

        public Algorithm(MinerBaseType minerBaseType, AlgorithmType primaryAlgorithmPoolID, string _AlgorithmNameCustom = "WOW!UnknownAlgo", bool enabled = true, bool hidden = false)
        {
            PrimaryAlgorithmPoolID = primaryAlgorithmPoolID;
            AlgorithmName = AlgorithmNames.GetName(PrimaryAlgorithmPoolID);
        ...
        public double CurPayingRatio
        {
            get
            {
                ...
                AlgorithmType _PrimaryAlgorithmPoolID = PrimaryAlgorithmPoolID;
                AlgorithmSwitchingManager.MostProfitableCoin paying = new();
                if (AlgosProfitData.TryGetPaying(_PrimaryAlgorithmPoolID, out paying))
                {
        ...
        public double CurSecondPayingRatio
        {
            get
            {
                ...
                var ratio = International.GetText("BenchmarkRatioRateN_A");
                if (AlgosProfitData.TryGetPaying(SecondaryAlgorithmPoolID, out paying2))
                {
        ...
        public virtual double CurPayingRate
        {
            get
            {
                ...
                if (BenchmarkSpeed > 0 && AlgosProfitData.TryGetPaying(_PrimaryAlgorithmPoolID, out var paying))
                {
        ...
        public virtual double CurSecondPayingRate
        {
            get
            {
                ...
                if ( BenchmarkSecondarySpeed> 0 && AlgosProfitData.TryGetPaying(SecondaryAlgorithmPoolID, out var paying))
                {
                    //paying = ExchangeRateApi.ConvertBTCToPayoutCurrency(paying);
            ...
            {
                var rate = International.GetText("BenchmarkRatioRateN_A");
                if (BenchmarkSpeed > 0 && AlgosProfitData.TryGetPaying(PrimaryAlgorithmPoolID, out var paying))
                {
                    //paying = ExchangeRateApi.ConvertBTCToPayoutCurrency(paying);
                    //double.TryParse(value, out var valueBench);
                    var payingRate = value * paying.profit * Mult;
        ...
        public string BenchmarkSpeedString()
        {
            ...
            if (BenchmarkSpeed > 0)
            {
                return Helpers.FormatDualSpeedOutput(BenchmarkSpeed, 0, 0,  PrimaryAlgorithmPoolID);
            }
        ...
        public string SecondaryBenchmarkSpeedString()
        {
            ...
            {
                return Helpers.FormatDualSpeedOutput(BenchmarkSecondarySpeed, 0, 0, DualZPoolID);
            }
        ...
        {
            profits.TryGetValue(PrimaryAlgorithmPoolID, out var paying);
            profits.TryGetValue(SecondaryAlgorithmPoolID, out var payingSecond);
            double _paying = 0d;
            ...
            var _c = Stats.Stats.CoinList.Find(a => (a.symbol.ToLower() == CurrentMiningCoin.ToLower()) &&
                                    (a.algo.ToLower() == PrimaryAlgorithmPoolID.ToString().ToLower()));
            ...
            var _c2 = Stats.Stats.CoinList.Find(a => a.algo.ToLower() == SecondaryAlgorithmPoolID.ToString().ToLower());
            ...
            if (ConfigManager.GeneralConfig.with_power)
            {
                ...
                CurrentProfitWithPower = CurrentProfit - power;
                MostProfitWithPower = MostProfit - power;
            }
        }
        ...
        protected void SubtractPowerFromProfit()
        {
            ...
            power *= 24 * Form_Main._factorTimeUnit;
            // Now we subtract from profit, which may make profit negative
            Helpers.ConsolePrint("---", "CurrentProfitWithPower: " + CurrentProfitWithPower.ToString() + " " +
